...
.controller('CustomerController', function ($scope, $http, $location, $window) {
    ...
    $scope.getCustomer = function (custModel) {
        var url = window.location.protocol + '//' + window.location.host + window.location.pathname;
        //console.log('URL: ' + url);
        $http.get(url +'Home/GetbyID/' + custModel.Id)
        .success(function (data, status, headers, config) {
            //debugger;
    ...
    $scope.deleteCustomer = function (custModel) {
        //debugger;
        var IsConf = confirm('You are about to delete ' + custModel.CustomerName + '. Are you sure?');
        if (IsConf) {
            var url = window.location.protocol + '//' + window.location.host + window.location.pathname;
            $http.delete(url + 'Home/Delete/' + custModel.Id)
            .success(function (data, status, headers, config) {
                if (data.success === true) {
                    $scope.message = custModel.CustomerName + ' deleted from record!!';
                    $scope.result = "color-green";
                    getallData();
                    console.log(data);
                }
